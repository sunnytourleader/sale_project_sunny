...
def meeting_report():
    ...
    # Fetch Dropdown configs from meta_data table
    ...
    if inspect(engine).has_table('meta_data'):
        try:
            ...
            if 'destinations' in df_meta.columns: 
                meta_data['destinations'] = sorted(list(set(str(d).strip() for d in df_meta['destinations'] if str(d).strip() and str(d).strip().upper() != 'ALL DESTINATION')))
...
# ==========================================
# ROBUST BULLETPROOF SAVE TOUR ENDPOINT
# ==========================================
...
def save_tour():
    ...
    try:
        with engine.begin() as conn:
            ...
            # Smart Case insensitive mapping
            col_map_lower = {c.lower(): c for c in existing_cols}
            
            # Map known alias variations
            if 'destinations' in col_map_lower and 'destination' not in col_map_lower:
                col_map_lower['destination'] = col_map_lower['destinations']
            if 'ticket ' in col_map_lower and 'ticket' not in col_map_lower:
                col_map_lower['ticket'] = col_map_lower['ticket ']
            if 'remain ' in col_map_lower and 'remain' not in col_map_lower:
                col_map_lower['remain'] = col_map_lower['remain ']
            ...
            for k, v in cols_mapping.items():
                ...
                if k_low in col_map_lower:
                    ...
                    if v is None or str(v).strip() == '':
                        update_cols[actual_col_name] = None
                    else:
                        if any(x in k_low for x in ['day', 'booked', 'sold', 'remain', 'price', 'capacity']):
                            try:
                                update_cols[actual_col_name] = float(str(v).replace(',', '').replace('$', '').strip())
                            except ValueError:
                                update_cols[actual_col_name] = 0.0
                        else:
                            update_cols[actual_col_name] = str(v).strip()
            
            if is_edit:
                # Find the actual primary key column name in the database (Tour_Code or Destinations)
                identifier_col = None
                ...
                if not identifier_col:
                    return jsonify({"error": "No valid identifier column found in database to perform update."}), 400
                ...
                # Fallback if exact date string didn't match
...
def ocr_tool_page():
    ...
    if request.method == 'POST':
        ...
        except: return render_template('ocr_tool.html', error="Failed to read data.", quotes=quotes)

        results = [{"No": "", "Customer Name": i.get("Customer Name", ""), "Contact Number": format_phone_number(i.get("Contact Number", "")), "Contacted Date": i.get("Contacted Date", ""), "Source": i.get("Source", "PAGE"), "Type": i.get("Type", "NEW"), "Destination": i.get("Destination", ""), "Zone": i.get("Zone", ""), "Pax": i.get("Pax", 1), "Depature Date": i.get("Depature Date", ""), "Status": i.get("Status", ""), "Next Follow up Date": i.get("Next Follow up Date", ""), "Why Failed": i.get("Why Failed", ""), "Sale Rep": i.get("Sale Rep", ""), "Success(%)": i.get("Success(%)", ""), "Noted": i.get("Noted", ""), "Department": i.get("Department", "")} for i in extracted_items]
        ...
        output = io.BytesIO()
        with pd.ExcelWriter(output, engine='openpyxl') as writer: df_new.to_excel(writer, index=False, sheet_name='Chat Data')
        return render_template('ocr_tool.html', results=results, excel_b64=base64.b64encode(output.getvalue()).decode('utf-8'), quotes=quotes)
